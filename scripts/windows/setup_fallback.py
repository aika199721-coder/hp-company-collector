...
def main() -> int:
    ...
    try:
        configured = os.environ.get("HPCC_PYTHON", "").strip().strip('"')
        if configured and Path(configured).is_dir():
            configured = str(Path(configured) / "python.exe")
        bits = struct.calcsize("P") * 8
        diagnostics = [
            f"PATH: {os.environ.get('PATH', '')}",
            f"where python: {shutil.which('python') or ''}",
            f"HPCC_PYTHON: {configured}",
            f"最終採用Python: {sys.executable}",
            f"version: {sys.version_info[:2]}",
            f"bit数: {bits}",
        ]
        if configured and not Path(configured).is_file():
            return _write(log, "\n".join([*diagnostics, "HPCC_PYTHONの実体がありません。"]), 2)
        if sys.version_info[:2] != (3, 13) or bits != 64:
            return _write(log, "\n".join([*diagnostics, "64bit Python 3.13が必要です。"]), 2)
