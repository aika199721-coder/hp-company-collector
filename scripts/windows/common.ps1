# setupfix11.2.2.1-v1: canonical Windows PowerShell 5.1 launcher functions.
...
function Add-PythonCandidate {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)][System.Collections.ArrayList]$Candidates,
        [string]$Exe,
        [string[]]$Prefix = @(),
        [string]$Source
    )
...
function Get-PythonCandidates {
    ...
    if ($null -ne $command) {
        Add-PythonCandidate -Candidates $candidates -Exe $command.Source `
