...
2. `setup.bat`を1回ダブルクリックし、`Press any key to continue`が表示されるまで待ちます。
   管理者として実行する必要はありません。
...
GitHub ActionsのArtifact名には生成元commit SHAが付きます。ZIP内の`setup.bat`には
`setupfix8-v1`が含まれ、`cmdcmdline`判定は含まれません。CIは現在のcheckoutとZIP内の
`setup.bat`、`scripts/windows/setup.ps1`のSHA256が一致しない場合、Artifactを公開しません。
