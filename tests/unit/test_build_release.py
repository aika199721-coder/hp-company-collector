...
def test_ci_uploads_exact_release_filename() -> None:
    ...
    assert "Verify canonical setup content and SHA256" in workflow
...
def test_release_all_scripts_match_source_hashes(tmp_path: Path) -> None:
    """Prevent stale BAT, PowerShell, or Python files from reaching the artifact."""
    root = Path(__file__).resolve().parents[2]
    output = build_release(tmp_path / "release.zip")

    with zipfile.ZipFile(output) as archive:
        scripts = [
            name for name in archive.namelist() if name.endswith((".bat", ".ps1", ".py"))
        ]
        for name in scripts:
