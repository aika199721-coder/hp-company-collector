...
def test_setupfix8_launcher_is_release_marked_and_does_not_use_cmdcmdline() -> None:
    ...
    assert "setupfix8-v1" in batch
    assert "setupfix8-v1" in powershell
    assert "cmdcmdline" not in batch.lower()
    assert "-PauseOnExit" in batch
