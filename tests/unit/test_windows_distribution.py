...
def test_setup_launcher_is_release_marked_and_does_not_use_cmdcmdline() -> None:
    """Prevent the obsolete double-click heuristic from returning to the artifact."""
    batch = (ROOT / "setup.bat").read_text(encoding="utf-8")
    powershell = (ROOT / "scripts/windows/setup.ps1").read_text(encoding="utf-8")

    assert "setupfix11.2.2.1-v1" in batch
    assert "setupfix11.2.2.1-v1" in powershell
    assert "cmdcmdline" not in batch.lower()
    assert "Press any key to continue" in batch
