...
class ApplicationFactory:
    ...
    def build(self) -> ApplicationComponents:
        ...
        query_builder = self._get("query_builder", QueryBuilder())
        client = self._get("http_client", requests.Session())
        providers = self._get("providers", [BingRSSProvider(client=client)])
        search_manager = self._get("search_manager", SearchManager(providers))
