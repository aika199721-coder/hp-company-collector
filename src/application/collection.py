...
class CollectionService:
    ...
    def run(
        ...
        audit_setter = getattr(components.search_manager, "set_run_id", None)
        if audit_setter is not None:
            audit_setter(run_id)
...
def _summary_dict(summary: PipelineSummary) -> dict[str, object]:
    ...
    return {
        ...
        "business_target_count": summary.business_target_count,
        "official_count": summary.official_count,
        "excluded_count": summary.excluded_count,
        "no_phone_count": summary.no_phone_count,
        "retry_count": summary.retry_count,
        "failed_count": summary.failed_count,
        "robots_denied_count": summary.robots_denied_count,
        "status_counts": {str(key): count for key, count in summary.status_counts.items()},
    }
