...
class PipelineCoordinator:
    ...
    def __init__(
        ...
        *,
        ...
        result_classifier: SearchResultClassifier | None = None,
        max_queries: int = 1,
    ...
    def run(self, condition: SearchCondition) -> PipelineSummary:
        ...
        criteria = SearchCriteria(condition.prefecture, condition.municipality, condition.industry)
        queries = self._query_builder.build_many(criteria)[: self._max_queries]
        candidate_limit = self._limits.max_candidates_per_condition
        outcomes: list[ProcessingResult] = []
        seen: set[str] = set()
        discovered = 0
        for query in queries:
            query_limit = min(candidate_limit, self._results_per_query or candidate_limit)
            for result in self._search_manager.search(query, query_limit):
                candidate = CandidateUrl(
                    result.url, query, result.provider, result.rank, result.title, result.snippet
                )
                search_result_id = self._repository.record_candidate(condition_id, candidate)
                task_key = _task_key(candidate.url)
                if task_key in seen:
                    continue
                seen.add(task_key)
                discovered += 1
                decision = (
                    self._result_classifier.classify(result, condition.industry)
                    if self._result_classifier
                    else None
                )
                if decision is not None and not decision.accepted:
                    self._repository.save_prefetch_exclusion(
                        candidate.url, decision.reason or "prefetch_excluded"
                    )
                ...
                self._progress.enqueue(task_key, payload)
                if discovered >= candidate_limit:
                    break
            if discovered >= candidate_limit:
                break
        target_limit = min(
        ...
        summary = _summarize(discovered, tuple(outcomes), self._limits)
...
def _counts_as_business_target(result: ProcessingResult, limits: PipelineLimits) -> bool:
...
def _summarize(
    ...
    return PipelineSummary(
        ...
        sum(
            bool(result.scoring_result and result.scoring_result.is_official) for result in outcomes
        ),
