...
_NARRATIVE_SIGNALS = ("。", "です", "ます", "属する", "周囲")
_LOCATION_DETAIL = re.compile(r"(?:\d|[一二三四五六七八九十]+丁目|番地|番\b|号)")
...
def _from_text(text: str, source: str) -> ExtractedAddress | None:
    ...
    candidate = address_match.group(0)
    if any(signal in candidate for signal in _NARRATIVE_SIGNALS):
        return None
    if source in {"body", "footer"} and not postal_match and not _LOCATION_DETAIL.search(candidate):
        return None
